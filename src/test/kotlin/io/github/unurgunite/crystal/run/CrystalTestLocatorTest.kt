...
class CrystalTestLocatorTest : BasePlatformTestCase() {
    ...
    private fun locate(path: String, protocol: String = CrystalTestLocator.PROTOCOL) =
        CrystalTestLocator.INSTANCE.getLocation(
            protocol, path, project, GlobalSearchScope.allScope(project)
        )
     ...
     */
    private fun writeTempSpec(name: String, content: String): String {
        val dir = java.nio.file.Files.createTempDirectory("crystal-locator-test").toFile()
        tempDirs.add(dir)
    ...
    fun testValidLineNavigatesToElement() {
        val path = writeTempSpec("sample_spec.cr", """
describe "sample" do
  it "works" do
  end
end
        ...
        assertEquals(
            ...
            document.getLineNumber(psiLocation!!.psiElement.textRange.startOffset), 1
        )
